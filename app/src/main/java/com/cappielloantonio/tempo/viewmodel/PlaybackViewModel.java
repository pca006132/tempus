...
public class PlaybackViewModel extends ViewModel {
    public class State {
        public State() {}
        public State(String id, boolean isPlaying, int position) {
    ...
    public void update(String songId, boolean playing, int position) {
        State current = state.getValue();
        if (current == null || !Objects.equals(current.currentSongId, songId) || current.isPlaying != playing || current.currentSongPosition != position)
            state.postValue(new State(songId, playing, position));
    }
